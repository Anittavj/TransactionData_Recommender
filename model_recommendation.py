...
def get_feature_data():
    df = pd.read_csv('/home/user3/Downloads/online_transaction_data.csv')
...
def generate_recommendations(idx, train, df_orig):
    ...
    print('ORIGINAL:')
    print(train.iloc[idx,:]['Description'])
    print(df_orig.iloc[original_index,:][['Description']])
    ...
    print(temp[temp['Description']][['CustomerID','Country', 'UnitPrice']])
    print('----------------------------------------------\n')
    pass
...
def vectorize_text_feature(df, feature, num_vec_features):
    ...
    df[feature + '_stemmed'] = df[feature].map(lambda x: ' '.join([stemmer.stem(y) for y in x.split(' ') if y.isalpha()]))
    ...
    col = [feature+ '_v_'+str(i) for i in range(num_vec_features)]
